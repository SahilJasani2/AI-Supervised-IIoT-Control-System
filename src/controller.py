...
pid = PID(Kp=0.8, Ki=0.1, Kd=0.05, setpoint=1500.0)
...
def on_ai_command(client, userdata, msg):
    ...
    'userdata' will be the asyncio event loop from the main thread.
    """
    loop = userdata
    try:
        ...
        if action == "enter_safe_mode":
            ...
            # --- FIXED: Use the event loop passed via userdata ---
...
async def main():
    ...
    # --- FIXED: Get the running event loop from the main thread ---
    main_loop = asyncio.get_running_loop()

    # --- MQTT Client Setup ---
    # Pass the main event loop to the client as userdata
    mqtt_client = mqtt.Client(userdata=main_loop)
